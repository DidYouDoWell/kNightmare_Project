...
public class Player : Actor {
	...
	void Start () {
        ...
        this.transform.rotation = Quaternion.Euler(0, 70f, 0);
    }
	...
	public void LoadStatus()
	{
		...
		{
			HPStatus = PlayerPrefs.GetInt("Plus" + eStatusData.MAX_HP.ToString()) -1;   //basicStatusUp에서 1을 증가시켜 작업하므로.
			basicStatusUp(eStatusData.MAX_HP);
		}
		if (PlayerPrefs.HasKey("Plus" + eStatusData.ATTACK.ToString()))
		{
			APStatus = PlayerPrefs.GetInt("Plus" + eStatusData.ATTACK.ToString())-1;
			basicStatusUp(eStatusData.ATTACK);
		}
		if (PlayerPrefs.HasKey("Plus" + eStatusData.MAX_HP.ToString()))
		{
			DPStatus = PlayerPrefs.GetInt("Plus" + eStatusData.DEFENCE.ToString())-1;
			basicStatusUp(eStatusData.DEFENCE);
		}
	...
	new void Update() {
        ...
        if (GameManager.Instance.GAME_START)
        {
            if(this.transform.position.x < 0.0f)
                this.transform.localPosition += new Vector3(0.03f, 0, 0);
            ProcessWalk();
            Invoke("StartGame", 2f);
        }
        ...
        if (SELF_CHARACTER.TargetComponenet.OBJECT_STATE == eBaseObjectState.STATE_DIE && playerAlive==true)
		{
            ...
            UI_Tools.Instance.ShowUI(eUIType.PF_UI_GAMEOVER);
		}
	...
	public void Stun(bool anim = false)
	{
	...
	{
		PlayerPrefs.SetFloat("RemainedPlayerHP",(float)SELF_CHARACTER.CURRENT_HP);
	}
	...
	protected virtual void ProcessAttack()
	{
        ...
        if(ComboSkillSet.IS_START_SKILL == true)
            this.ThrowEvent(ConstValue.EventKey_SelectSkill, 1);
        else

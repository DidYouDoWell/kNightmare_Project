...
public class ActorManager : MonoSingleton<ActorManager>
{
   ...
   public bool returnTitle = false;
    ...
    public void RemoveActor(Actor actor, bool bDelete = false)
    {
		...
		{
			Debug.Log("GameClear");
			UI_Tools.Instance.ShowUI(eUIType.PF_UI_GAMECLEAR);
			return;
    ...
    {
        //플레이어가 무적상태 라면, 탐색안함.
        if (GameManager.Instance.PLAYER_ACTOR.IS_INVUL 
            || GameManager.Instance.PLAYER_ACTOR.GetComponent<Player>().ComboSkillSet.DrawSkill.IS_DRAW_SKILL)
        {
            radius = 0;
        }
        eTeamType teamType =

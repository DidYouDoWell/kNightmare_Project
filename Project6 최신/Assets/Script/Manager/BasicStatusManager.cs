...
public class BasicStatusManager : MonoSingleton<BasicStatusManager> {
	...
	public BasicStatusTemplateData Get(string _key, int _index)
	{
		...
		////eStatusData type //스텟을 올릴때 저장하는 방식
		//PlayerPrefs.SetInt("Plus"+_key, _index);
	...
	{
		PlayerPrefs.SetInt("Plus" + eStatusData.MAX_HP, (GameManager.Instance.PLAYER_ACTOR as Player).HPStatus);
		PlayerPrefs.SetInt("Plus" + eStatusData.ATTACK, (GameManager.Instance.PLAYER_ACTOR as Player).APStatus);
		PlayerPrefs.SetInt("Plus" + eStatusData.DEFENCE, (GameManager.Instance.PLAYER_ACTOR as Player).DPStatus);
	}
